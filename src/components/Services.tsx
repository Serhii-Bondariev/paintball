...
const Services = () => {
  ...
  const packages = [
    ...
    },
    {
      title: 'VIP',
      price: 'від 1200 грн',
      features: ['4 години гри', '300 кульок', 'Преміум спорядження', 'Всі локації', 'Персональний інструктор', 'Фото та відео з гри'],
      isPopular: false,
      color: 'bg-neutral-100 hover:bg-neutral-200 text-rose-600',
      borderColor: '',
    },
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
          {services.map((service, index) => (
            <div
              key={index}
              className="bg-white p-6 rounded-lg shadow-md transition-transform hover:-translate-y-1 border-t-4 border-rose-500"
            >
              <div className="mb-4">{service.icon}</div>
              <h3 className="text-xl font-semibold mb-3 text-rose-600">{service.title}</h3>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {packages.map((pkg, index) => (
            <div
              key={index}
              className={`bg-white rounded-lg shadow-lg overflow-hidden transition-transform hover:-translate-y-1 relative ${pkg.borderColor}`}
            >
              {pkg.isPopular && (
              ...
              <div className="p-6">
                <h4 className="text-xl font-bold mb-2 text-rose-600">{pkg.title}</h4>
                ...
                <Link
                  to={`/contact?package=${encodeURIComponent(pkg.title)}&message=${encodeURIComponent(
                    `Я хочу замовити пакет ${pkg.title}: ${pkg.features.join(', ')}`
                  )}`}
                  className={`w-full py-3 rounded-lg font-medium transition-colors block text-center ${pkg.color}`}
                  aria-label={`Обрати пакет ${pkg.title}`}
                >
                  Обрати пакет
                </Link>
              </div>
...
//             </div>)}
//         </div>
//       </div>import { Users2Icon, TrophyIcon, CalendarIcon, ShirtIcon } from 'lucide-react';
...
//   const handlePackageSelect = (pkg: { title: string; features: string[] }) => {
//     const message = `Я хочу замовити пакет ${pkg.title}: ${pkg.features.join(', ')}`;
//     navigate(`/contact?package=${encodeURIComponent(pkg.title)}&message=${encodeURIComponent(message)}`);
//   };
...
//                   className={`w-full py-3 rounded-lg font-medium transition-colors ${pkg.color}`}
//                   aria-label={`Обрати пакет ${pkg.title}`}
//                 >
//                   Обрати пакет
//                 </button>
